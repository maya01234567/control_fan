...
/*
đưa url,api_key và version vào dữ liệu chung semaphore
*/
void otadrive_setInfo(char *apiKey, char *current_version)
{
    ...
    uint64_t _chipmacid = 0LL;                           // LL là long long 64 bit
    esp_efuse_mac_get_default((uint8_t *)(&_chipmacid)); // lấy địa chỉ MAC tại vùng nhớ chỉ đọc không xóa EFUSE
...
otadrive_result otadrive_updateFirmware()
{
    ...
    while (1)
    {
        ...
        {
            lock = 0;
            esp_http_client_open(client, 1024);
            size = esp_http_client_fetch_headers(client);
            ESP_LOGI(TAG, "Firmware Update Length: %d", size);
        }
...
char *otadrive_get_event_str(otadrive_event_e event)
{
    ...
    case WIFI_CONNECTTED:
        return "WIFI OTA CONNECTTED";
    case WIFI_DISCONNECTTED:
        return "WIFI OTA DISCONNECTED";
    }
    return "Unknown Event";
}

bool otadrive_timeTick(uint32_t seconds)
{
